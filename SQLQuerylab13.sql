...
VALUES
  (1, '2022-04-19', 'Nguyễn Văn Chiến ', 1, 2, 15000),
  (2, '2022-04-20', 'Trần Thị Bích Huyền ', 2, 1, 12000),
  (3, '2022-04-20', 'Lê Văn Cường ', 3, 3, 5000),
  (4, '2022-04-21', 'Phạm Thị Duyên ', 4, 5, 2000),
  (5, '2022-04-21', 'Đỗ Văn Lương ', 5, 2, 10000);
...
-------Câu 3c-------
CREATE TRIGGER trg_nhatkybanhang1_insert
ON NHATKYBANHANG
FOR INSERT
AS
BEGIN
...
---------Câu 3d-----------
CREATE TRIGGER trg_nhatkybanhang2_update
ON NHATKYBANHANG
FOR UPDATE
AS
BEGIN
...
-----------Câu 3e--------
CREATE TRIGGER trg_nhatkybanhang3_delete
ON NHATKYBANHANG
FOR DELETE
AS
BEGIN
...
--------Câu 3f------------
CREATE TRIGGER trg_nhatkybanhang4_update
ON NHATKYBANHANG
FOR UPDATE
AS
BEGIN
...
INSERT INTO NHATKYBANHANG (stt, ngay, nguoimua, mahang, soluong, giaban)
VALUES
(6, '2022-04-22', 'Nguyễn Thị Mỹ Huyền ', 1, 3, 15000)
SELECT * FROM MATHANG
-- Test cho câu 3b
...
INSERT INTO NHATKYBANHANG (stt, ngay, nguoimua, mahang, soluong, giaban)
VALUES
(7, '2022-04-23', 'Trương Văn Giáp', 2, 10, 12000)
SELECT * FROM MATHANG
-- Test cho câu 3d
